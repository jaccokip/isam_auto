...
logging.basicConfig()
logger = logging.getLogger("UsernameTokentoSAML20")
...
def usage():    
    logger.info("     Usage: UsernameTokentoSAML20.py -configure All")
...
if __name__ == '__main__':
    ...
    logger.info("Configuring the UsernameTokentoSAML20 chain")
    ...
    try:
        stsModuleInstancesList.index("default-username")
    ...
    chainTemplateName = "UsernameTokenToSAML20"
    ...
    for i in range(noOfChainTemplates):
        if (jsonStsChainTemplates[i]["name"] == chainTemplateName):
            ...
            if(len(chainItems) == 3 and chainItems[0]['id'] == 'default-username' and chainItems[0]['mode'] == 'validate' and 
    ...
    if(chainTemplateId == None):
        ...
        chainTemplateId = ipFedConfig.createSTSModuleChainTemplateUTtoSAML2(uuidChainItemstsuu1, uuidChainItemMap, uuidChainItemstsuu2)
         
    else:
        logger.debug("Chain template exists and the id is "+chainTemplateId)
      
    chainName = "UsernameTokenToSAML20Chain";
    chainIssuerAddress = "http://issuer/username";
    ...
    if(chainListId == None):
        ...
        ipFedConfig.createSTSModuleChainUTtoSAML2(uuidChainItemstsuu1, uuidChainItemMap, uuidChainItemstsuu2, chainTemplateId, mapJsRuleReference='ut_to_saml20.js')
    else:
